...
DEFAULT_CONFIG = {
    "bot_token":          "8837664705:AAH3fFBMMYuDyR_ymCeTPneCy3ZGKAuSR5c",
    "admin_id":           7884194046,
    "admin_set":          False,
    ...
    # Force-join channels
    "channels": [
        {"name": "🌺 MAIN", "username": "me king 👑 paribar", "url": "https://t.me/Mr_King_Paribar"},
        {"name": "🤖 JOIN", "username": "-1004477501629", "url": "https://t.me/Mr_Monirul_Community"}
    ],

    "qr_code_url": "@Tra_der_habib",
    "payment_contact": "@@Tra_der_habib",
    "bot_username": "Habib_smm_pro_bot"
}
...
def send_welcome(message):
    ...
    bot.send_message(
        ...
        "𝙁𝙤𝙧 𝙇𝙖𝙩𝙚𝙨𝙩 𝙐𝙥𝙙𝙖𝙩𝙚𝙨 & 𝙍𝙚𝙜𝙪𝙡𝙖𝙩𝙞𝙤𝙣ս 𝙔𝙤𝙪 𝙉𝙚𝙚𝙙 𝙏𝙤 𝙅𝙤𝙞𝙣 𝙊𝙪্রে 𝘾𝙃𝗔𝗡𝗡𝗘𝗟𝙎:",
...
def add_funds(message):
    ...
    bot.send_photo(
        user_id,
        cfg["qr_code_url"],
        caption=f"📌 SCAN QR OR SEND MONEY 0171556882\n\nFor help DM {cfg['payment_contact']}"
    )
...
# ─────────────────────────────────────────────────────────────
#  POLLING WITH WEB SERVER FOR RENDER PORT BINDING
# ─────────────────────────────────────────────────────────────
import threading
from http.server import SimpleHTTPRequestHandler, HTTPServer

# রেন্ডার পোর্ট বাইন্ডিং সমস্যা সমাধানের জন্য ছোট ওয়েব সার্ভার
def run_web_server():
...
# ব্যাকগ্রাউন্ড থ্রেডে ওয়েব সার্ভারটি আগে চালু করা
threading.Thread(target=run_web_server, daemon=True).start()

# এরপর বটের পোলিং লুপ শুরু করা
print("🤖 Bot started Powered by ETHBD")
while True:
    try:
